...
// 모바일 하단 탭에 표시할 주요 5개 항목
const mobileNavItems = navItems.filter((item) =>
  ['/', '/experiments', '/calendar', '/references', '/settings'].includes(item.path)
)
...
function NavItem({ item, mobile = false }) {
  return (
    ...
    >
      {item.icon}
      <span>{mobile ? item.label.replace('데이터 브라우저', '브라우저') : item.label}</span>
    </NavLink>
  )
}
    ...
    <>
      ...
      <aside className="hidden md:flex flex-col w-56 shrink-0 border-r border-gray-200 bg-white h-screen sticky top-0">
        <div className="px-4 py-5 border-b border-gray-200">
          <h1 className="text-base font-bold text-gray-900">Lab Notebook</h1>
        </div>
      ...
      {/* 모바일 하단 탭바 */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 z-50 flex justify-around px-2 safe-area-pb">
